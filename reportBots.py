...
def reportBots(TARGET_USER, twitter_api, probabilityCut):
    ...
    users = c.execute('SELECT * FROM {} WHERE capUniversal >= ? AND (reported IS NULL AND (lastCheck!=0 or lastCheck IS NULL))'.format(
            TARGET_USER), [probabilityCut]).fetchall()
    print("Collected " + len(users).__str__() + " possible bots")
    for user in users:
        try:
            twitter_api.report_spam(user_id=user[0], screen_name=user[1])
